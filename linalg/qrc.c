      ...
      for (i = 0; i < GSL_MIN (M, N); i++)
        {
          gsl_vector_complex_const_view c = gsl_matrix_complex_const_column (QR, i);
          gsl_vector_complex_const_view h = gsl_vector_complex_const_subvector (&(c.vector), i, M - i);
          gsl_matrix_complex_view m = gsl_matrix_complex_submatrix(A, i, 0, M - i, A->size2);
          gsl_complex ti = gsl_vector_complex_get (tau, i);
          gsl_complex ti_conj = gsl_complex_conjugate(ti);
          gsl_linalg_complex_householder_hm (ti_conj, &(h.vector), &(m.matrix));
        }
      return GSL_SUCCESS;
    }
}
...
int
gsl_linalg_complex_QR_matQ (const gsl_matrix_complex * QR, const gsl_vector_complex * tau, gsl_matrix_complex * A)
{
  const size_t M = QR->size1;
  const size_t N = QR->size2;
